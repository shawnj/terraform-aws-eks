locals {
  ...
  #   local.node_group_defaults_defaults,
  ...
  node_groups_expanded = { for k, v in var.node_groups : k => merge(
    {
      desired_capacity        = local.node_groups_defaults["desired_capacity"]
      #iam_role_arn            = var.default_iam_role_arn
      instance_type           = local.node_groups_defaults["instance_type"]
      key_name                = local.node_groups_defaults["key_name"]
      launch_template_id      = local.node_groups_defaults["launch_template_id"]
      launch_template_version = local.node_groups_defaults["launch_template_version"]
      max_capacity            = local.node_groups_defaults["max_capacity"]
      min_capacity            = local.node_groups_defaults["min_capacity"]
      subnets                 = local.node_groups_defaults["subnets"]
    },
    var.node_groups_defaults,
    v,
  ) if var.create_eks }
